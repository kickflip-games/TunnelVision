...
public class FollowShip : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (TargetShip == null)
            return;
        _lerpPosition = Lerp(_lerpPosition, TargetShip.transform.position,
            Time.deltaTime * 32f);
        this.transform.position = Lerp(this.transform.position,
            TargetShip.transform.position + TargetShip.transform.forward * Offset.z +
            TargetShip.transform.up * Offset.y, Time.deltaTime * 8f);
